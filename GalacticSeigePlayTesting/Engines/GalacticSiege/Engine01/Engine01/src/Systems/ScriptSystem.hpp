...
public:
	...
	void CreateLuaBinding(sol::state& lua) {
		...
		lua.set_function("get_entity_center", GetEntityCenter);
		lua.set_function("get_current_scene_name", GetCurrentSceneName);
		lua.set_function("get_leaderboard_lines", GetLeaderboardLines);
		lua.set_function("get_last_score_message", GetLastScoreMessage);
	...
	void Update(sol::state& lua, double dt, int wH, int wW) {
		for (auto entity :  GetSystemEntiities()) {
			...
			if (script.updateBullets != sol::lua_nil) {
			...
			if (script.createEnemy1 != sol::lua_nil) {

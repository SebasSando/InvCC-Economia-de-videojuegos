...
/**
 * @brief Retrieves current mouse position from the controller manager.
 *
...
/**
 * @brief Returns the identifier of the currently active scene.
 *
 * @return Scene name like "level_01" or "level_02".
 */
std::string GetCurrentSceneName() {
	return Game::GetInstance().sceneManager->GetCurrentSceneName();
};
/**
 ...
 */
void SetRotation(Entity entity, double angle) {
	auto& transform = entity.GetComponent<TransformComponent>();
	transform.rotation = angle;
};
/**
 * @brief Calculates the center position of an entity based on its transform and sprite.
 *
 * @param entity The entity to evaluate.
 * @return A tuple containing the X and Y coordinates of the entity center.
 */
std::tuple<double, double> GetEntityCenter(Entity entity) {
	auto& transform = entity.GetComponent<TransformComponent>();
	double centerX = transform.position.x;
	double centerY = transform.position.y;
	if (entity.HasComponent<SpriteComponent>()) {
		auto& sprite = entity.GetComponent<SpriteComponent>();
		centerX += (sprite.width * transform.scale.x) / 2.0;
		centerY += (sprite.height * transform.scale.y) / 2.0;
	}
	return { centerX, centerY };
};
/**
 * @brief Updates the text stored in an entity's TextComponent.
 *
 * @param entity The entity whose text should be updated.
 * @param newText The new text to display.
 */
void SetText(Entity entity, const std::string& newText) {
...
/**
 * @brief Provides the leaderboard entries formatted for UI display.
 *
 * @return Vector with formatted strings describing each leaderboard entry.
 */
std::vector<std::string> GetLeaderboardLines() {
	auto entries = Leaderboard::GetInstance().GetEntries();
	std::vector<std::string> lines;
	if (entries.empty()) {
		lines.emplace_back("Sin puntuaciones registradas");
		return lines;
	}
	...
	for (const auto& entry : entries) {
		...
		oss << rank << ") " << entry.score << " pts - " << entry.outcome;
...
/**
 * @brief Retrieves the message describing the last recorded score.
 */
std::string GetLastScoreMessage() {
	return Game::GetInstance().registry->GetSystem<GameManagerSystem>().GetLastScoreMessage();
};
/**
...
void GoToScene(const std::string& sceneName) {
	...
	LogMessage(Logger::INFO, "SCENE SET: " + sceneName + ".");
};
/**
...
void BulletFactory(double playerX, double playerY) {
	...
	glm::vec2 playerCenter(static_cast<float>(playerX + 32.0), static_cast<float>(playerY + 32.0));
	float playerHalfWidth = 32.0f;
	float playerHalfHeight = 32.0f;
	if (player.HasComponent<TransformComponent>() && player.HasComponent<SpriteComponent>()) {
		auto& transform = player.GetComponent<TransformComponent>();
		auto& sprite = player.GetComponent<SpriteComponent>();
		playerCenter.x = transform.position.x + static_cast<float>(sprite.width * transform.scale.x) / 2.0f;
		playerCenter.y = transform.position.y + static_cast<float>(sprite.height * transform.scale.y) / 2.0f;
		playerHalfWidth = static_cast<float>(sprite.width * transform.scale.x) / 2.0f;
		playerHalfHeight = static_cast<float>(sprite.height * transform.scale.y) / 2.0f;
	}

	glm::vec2 mousePos(static_cast<float>(mouseX), static_cast<float>(mouseY));
	glm::vec2 direction = mousePos - playerCenter;
	...
	glm::vec2 velocity = direction * bulletSpeed;

	const float bulletScale = 0.5f;
	const float bulletWidth = 64.0f * bulletScale;
	const float bulletHeight = 64.0f * bulletScale;
	float spawnOffset = std::max(playerHalfWidth, playerHalfHeight);
	glm::vec2 bulletCenter = playerCenter + direction * spawnOffset;
	glm::vec2 bulletPosition = bulletCenter - glm::vec2(bulletWidth / 2.0f, bulletHeight / 2.0f);
	float angle = glm::degrees(atan2(direction.y, direction.x)) + 90.0f;
	...
	bullet.AddComponent<TransformComponent>(bulletPosition, glm::vec2(bulletScale, bulletScale), angle);
...
void Enemy1Factory(int windowHeight, int windowWidth) {
	...
	LogMessage(Logger::INFO, "Enemy type 1 spawned.");
}
/**
...
void Enemy2Factory(int windowHeight, int windowWidth) {
	...
	LogMessage(Logger::INFO, "Enemy type 2 spawned.");
}
/**
...
void Enemy3Factory(int windowHeight, int windowWidth) {
	...
	LogMessage(Logger::INFO, "Enemy type 3 spawned.");
}
/**
...
void ExtraLifeFactory(int windowHeigth, int windowWidth) {
	...
	LogMessage(Logger::INFO, "Extra life generated.");
}
/**
...
void NukeFactory(int windowHeigth, int windowWidth) {
	...
	LogMessage(Logger::INFO, "Nuke generated.");
}
/**
...
void BossAttack(double dirX, double dirY, double posX, double posY) {
	...
	bossBullet.AddComponent<HealthComponent>(2);
